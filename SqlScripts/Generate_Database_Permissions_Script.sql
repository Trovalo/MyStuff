...
UNION
SELECT 
CASE WHEN DATALENGTH(sid) = 28 AND type = 'S' /* users withrout login */ THEN ('IF NOT EXISTS (SELECT [name] FROM sys.database_principals WHERE [name] = ' + SPACE(1) + '''' + [name] + '''' + ') BEGIN CREATE USER ' + SPACE(1) + QUOTENAME([name]) + ' WITHOUT LOGIN WITH DEFAULT_SCHEMA = ' + QUOTENAME([default_schema_name]) + SPACE(1) + ', SID = ' + CONVERT(varchar(1000), sid) + SPACE(1) + ' END; ')
...
3.2 AS [-- RESULT ORDER HOLDER --]
UNION

SELECT 
  'IF NOT EXISTS (SELECT [name] FROM sys.database_principals WHERE [name] = ' + SPACE(1) + '''' + [name] + '''' + ') BEGIN CREATE ROLE ' + SPACE(1) + QUOTENAME([name]) + SPACE(1) +'END' AS [-- SQL STATEMENTS --],
3.3 AS [-- RESULT ORDER HOLDER --]
...
and [type] IN ('G', 'S', 'U', 'R') -- S = SQL user, U = Windows user, G = Windows group
 )
...
SELECT [-- SQL STATEMENTS --] AS statements
FROM permissions_query
